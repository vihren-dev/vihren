// Package litekit hosts a Temporal server backed by SQLite inside the current
// process. It powers platform/embeddedtemporal's persistent (file-backed) and
// ephemeral modes so a single binary can run durable workflows with no external
// Temporal deployment.
//
// VENDORED: this file is copied, nearly verbatim, from
// go.temporal.io/server/temporaltest/internal/lite_server.go (the LiteServer
// helper), which is not importable because it lives in an internal package. The
// only change is the package name. Re-sync it when bumping go.temporal.io/server.
//
// The MIT License
